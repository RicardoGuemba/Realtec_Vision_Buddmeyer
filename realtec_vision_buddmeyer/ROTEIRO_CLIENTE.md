...
- **Arquivo de log:** na pasta da aplicação, subpasta `logs`, arquivo `realtec_vision.log`  
  (caminho típico: `realtec_vision_buddmeyer\logs\realtec_vision.log`).
...
2. **Abra o arquivo de log** `logs\realtec_vision.log` e procure por `cip_error`. Lá estarão o IP, a porta e o nome da tag que falharam.
...
4. Execute:
   ```bat
   python realtec_vision_buddmeyer\scripts\test_cip_config_reload.py
   ```
...
| Ver qual IP foi usado   | Abrir `logs\realtec_vision.log` e procurar `cip_connecting`. |
