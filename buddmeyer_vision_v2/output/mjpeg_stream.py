...
class StreamFrameProvider:
    ...
    def get_jpeg_bytes(self) -> Optional[bytes]:
        ...
        Desenha bbox, centroide e labels no frame usando OpenCV.
        Thread-safe.
        """
        with self._lock:
            if self._frame is None:
                return self._last_jpeg  # Reutiliza último frame se disponível

            img = self._frame.copy()

            # Desenha detecções (estilo similar ao VideoWidget)
            if self._detections:
